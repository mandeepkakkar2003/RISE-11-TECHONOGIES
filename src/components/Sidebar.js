...
export default function Sidebar() {
  ...
  return (
    <aside className="bg-white border-r border-gray-200 flex flex-col h-screen w-64 overflow-y-auto">
      {/* Header */}
      <div className="h-16 flex items-center px-6 border-b">
        <div className="text-indigo-600 font-bold text-2xl md:text-4xl">Jur</div>
      </div>
        ...
        <ul className="space-y-1">
          {menuItems.map((item) => (
            <li key={item.name}>
              <Link 
                to={item.link} 
                className={`flex items-center px-6 py-2 text-sm font-medium hover:bg-indigo-50 transition 
